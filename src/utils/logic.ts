...
export function withDerived(task: Task): DerivedTask {
  return {
    ...
    roi:
      task.status === 'Done'
        ? computeROI(task.revenue, task.timeTaken)
        : null,
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // 2️⃣ Priority (lower weight = higher priority)
    const priorityDiff = a.priorityWeight - b.priorityWeight;
    if (priorityDiff !== 0) return priorityDiff;

    // 3️⃣ ROI (higher ROI first, nulls last)
