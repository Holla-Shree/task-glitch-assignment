...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const hasValidRoi = (roi: unknown): roi is number =>
  typeof roi === 'number' && !Number.isNaN(roi);
  const roiBuckets = [
    {
      label: '<200',
      count: tasks.filter(t => hasValidRoi(t.roi) && t.roi < 200).length,
    },
    {
      label: '200–500',
      count: tasks.filter(t => hasValidRoi(t.roi) && t.roi >= 200 && t.roi <= 500).length,
    },
    {
      label: '>500',
      count: tasks.filter(t => hasValidRoi(t.roi) && t.roi > 500).length,
    },
    {
      label: 'N/A',
      count: tasks.filter(t => !hasValidRoi(t.roi)).length,
    },
  ];
